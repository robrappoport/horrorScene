...
public class GazeRaycast : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(myRay, out myRayHit, 100f))
        {
            ...
            // after 1 second of looking, timeLookedAt == 1.0f

            timeLookedAt += Time.deltaTime;
            // once we hit something, we should spin it, to show that we're looking at it
            if (timeLookedAt > 1f)
            {
                if (myRayHit.collider.tag == "face")
                {
                    myRayHit.collider.GetComponent<faceMove>().isLookedAt = true;
                }
 
            }
            else
            {
                myRayHit.collider.GetComponent<faceMove>().isLookedAt = false;
            }
        }
        else
        {
